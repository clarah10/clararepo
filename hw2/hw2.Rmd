---
title: "hw2"
author: "jiyeon"
date: "September 12, 2016"
output: github_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

## 3.2.1 Exercises

1. As you can see in the below, the ggplot gives the empty plot. This is because the "mpg" data include more than two varibles. 

```{r cars}
library(ggplot2)
library(tibble)
ggplot(data = mpg)
```

2. According to the help, drv variable has three categories. 'f' means front-wheel drive, 'r' means rear wheel drive, and '4' means 4wd. 

3. 
```{r}
plot(mpg$hwy, mpg$cyl)
```

4. This command does not work, giving an error. This is because x and y variables are non-numeric variables.

## 3.3.1 Exercises
1. The following command gives the blue points. Since the 'aes' command generates how variables in the data are mapped to visual 'properties' of geoms, the color command in aes should be the function of x or y. But we just need to draw blue points here. So we should move out the color command from aes command. 
```{r}
ggplot(data = mpg) + 
  geom_point(mapping = aes(x = displ, y = hwy), color = "blue")
```

2.
categorical: manufacturer, model, trans, drv, fl, class
continuous: displ
discrete: year, cyl, cty, hwy
If you run the data, the head of the data shows the types of variables; <chr>, <dbl>, <int>
<chr> (character) is a categorical variable, <dbl> (double precision) is a continuous variable, and <int> (integer) is a discrete variables.

3.
```{r}
ggplot(data=mpg)+geom_point(mapping=aes(x=cty, y=hwy, color=displ))
ggplot(data=mpg)+geom_point(mapping=aes(x=cty, y=hwy, color=drv))
```

```{r}
ggplot(data=mpg)+geom_point(mapping=aes(x=cty, y=hwy, size=displ))
ggplot(data=mpg)+geom_point(mapping=aes(x=cty, y=hwy, size=drv))
```

```{r}
ggplot(data=mpg)+geom_point(mapping=aes(x=cty, y=hwy, shape=factor(as.integer(displ))))+scale_shape_manual(values=c(1:7))
ggplot(data=mpg)+geom_point(mapping=aes(x=cty, y=hwy, shape=drv))
```

4.
```{r}
ggplot(data=mpg)+geom_point(mapping=aes(x=cty, y=hwy, color=drv, shape=drv))
ggplot(data=mpg)+geom_point(mapping=aes(x=cty, y=hwy, color=fl, shape=drv))
```

5.
```{r}
ggplot(data=mpg)+geom_point(mapping=aes(x=cty, y=hwy, color=displ), shape=21, stroke=2)
```

6. 
```{r}
ggplot(data=mpg)+geom_point(mapping=aes(x=cty, y = hwy, shape=displ<5))
```

## 3.5.1 Exercises
1.
```{r}
ggplot(data = mpg)+geom_point(mapping=aes(x=cty, y=hwy))+facet_wrap(~displ)
```

2.
```{r}
ggplot(data = mpg)+geom_point(mapping=aes(x=cty, y=hwy))+facet_grid(drv~cyl)
```

3.
```{r}
ggplot(data = mpg) + 
  geom_point(mapping = aes(x = displ, y = hwy)) +
  facet_grid(drv ~ .)

ggplot(data = mpg) + 
  geom_point(mapping = aes(x = displ, y = hwy)) +
  facet_grid(. ~ cyl)
```

4.
```{r}
ggplot(data = mpg) + 
  geom_point(mapping = aes(x = displ, y = hwy)) + 
  facet_wrap(~ class, nrow = 2)
```

5.

6.

## 3.6.1 Exercises
1.
geom_line
geom_box
geom_histogram
geom_area

2.
```{r}
ggplot(data = mpg, mapping = aes(x = displ, y = hwy, color = drv)) + 
  geom_point() + 
  geom_smooth(se = FALSE)
```

3.

4.

5.
```{r}
ggplot(data = mpg, mapping = aes(x = displ, y = hwy)) + 
  geom_point() + 
  geom_smooth()

ggplot() + 
  geom_point(data = mpg, mapping = aes(x = displ, y = hwy)) + 
  geom_smooth(data = mpg, mapping = aes(x = displ, y = hwy))
```

6.
```{r}
ggplot(data = mpg, mapping = aes(x = displ, y = hwy)) + 
  geom_point() + 
  geom_smooth(aes(group = drv), se = FALSE)
ggplot(data = mpg, mapping = aes(x = displ, y = hwy)) + 
  geom_smooth(aes(group = drv), se = FALSE) +
  geom_point()
ggplot(data = mpg, mapping = aes(x = displ, y = hwy, color = drv)) + 
  geom_point() + 
  geom_smooth(se = FALSE)
ggplot(data = mpg, mapping = aes(x = displ, y = hwy)) + 
  geom_point(aes(color = drv)) + 
  geom_smooth(se = FALSE)
ggplot(data = mpg, mapping = aes(x = displ, y = hwy)) + 
  geom_point(aes(color = drv)) +
  geom_smooth(aes(linetype = drv), se = FALSE)
ggplot(data = mpg, mapping = aes(x = displ, y = hwy)) + 
  geom_point(size = 4, colour = "white") + 
  geom_point(aes(colour = drv))
```

## 3.7.1 Exercises
1.
```{r}
ggplot(data = diamonds) + 
  geom_bar(mapping = aes(x = cut, y = ..prop..))
```


```{r}
ggplot(data = diamonds) + 
  geom_bar(mapping = aes(x = cut, y = ..prop..,group=1))
```

2.
According to help, by default, geom_bar uses stat="count" which makes the height of the bar proportion to the number of cases in each group (or if the weight aethetic is supplied, the sum of the weights). 

## 3.8.1 Exercises 
1.
overlapping problem
```{r}
ggplot(data = mpg, mapping = aes(x = cty, y = hwy)) + 
  geom_point()
```

2.
```{r}
ggplot(data = mpg, mapping = aes(x = cty, y = hwy)) + 
  geom_point() + geom_jitter()
```


```{r}
ggplot(data = mpg, mapping = aes(x = cty, y = hwy)) + 
  geom_point() + geom_count()
```

3.
```{r}
  ggplot(mpg, aes(drv,displ)) + 
      geom_boxplot()
```

## 3.9.1 Exercises
1.
```{r}
pie<-ggplot(data=mpg, aes(x = factor(1), fill = factor(cyl))) +
 geom_bar(width = 1)
pie+coord_polar(theta="y")
```

2.
Change axis labels and legend titles

3.

4.
```{r}
ggplot(data = mpg, mapping = aes(x = cty, y = hwy)) +
  geom_point() + 
  geom_abline() +
  coord_fixed()
```



## 4.4 Exercises
1. 'i' character

2.
```{r}
library(ggplot2)
library(dplyr)
```

typo:dota->data
```{r}
ggplot(data = mpg) + 
  geom_point(mapping = aes(x = displ, y = hwy))
```

typo: fliter->filter / cyl==8 (double equation for TRUE/FALSE judgement)
```{r}
mpg %>% 
  filter(cyl == 8)
```

typo:diamond->diamonds
```{r}
diamonds %>% 
  filter(carat > 3)
```

3.
Tools->Keyboard Shortcuts Help

## 5.2.4 Exercises
1. 
i.
```{r}
flights %>% filter(arr_delay >= 120)
```

ii.
```{r}
flights %>% filter(dest=="IAH"|dest=="HOU")
```

iii.
```{r}
flights %>% filter(carrier=="UA"|carrier=="AA"|carrier=="DL")
```

iv.
```{r}
flights %>% filter(month==7|month==8|month==9)
```

v.
```{r}
flights %>% filter(arr_delay >= 120 & dep_delay<=0)
```

vi.
```{r}
flights %>% filter(dep_delay >= 60 & dep_delay-30>=arr_delay)
```

vii.
```{r}
flights %>% filter(0<=dep_time & dep_time<=600)
```

2.
```{r}
flights %>% filter(between(flights$dep_time,0,600))
```

3.
8255
```{r}
flights %>% filter(is.na(dep_time))
```

dep_delay, arr_time, arr_delay also missing

involved in accident(cannot find blackbox as well)

4. 

## 5.3.1 Exercises
1.
```{r}
arrange(flights,desc(is.na(dep_time)))
```

2.
```{r}
arrange(flights,desc(dep_delay))
arrange(flights,desc(arr_delay))
```
Most delayed for depature and arrival respectively

```{r}
arrange(flights,dep_time)
```

3.
```{r}
arrange(flights,air_time)
```

4.
```{r}
arrange(flights,desc(distance))
arrange(flights,distance)
```

## 5.4.1 Exercises
1.
```{r}
select(flights, dep_time, dep_delay, arr_time, arr_delay)
select(flights, -year:-day,-sched_dep_time,-sched_arr_time,-carrier:-time_hour)
select(flights,matches("._."),-contains("sched"),-air_time,-time_hour)
```

2.
```{r}
select(flights, dep_time, dep_time, dep_delay,dep_delay, arr_time, arr_delay)
```

3.
```{r}
attach(flights)
vars<-c("dep_time", "dep_delay", "arr_time", "arr_delay")
select(flights,one_of(vars))
```

## 5.5.2 Exercises